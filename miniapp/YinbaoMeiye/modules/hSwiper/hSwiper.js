...
class hSwiper {
  constructor(props) {
    ...
    //this.nowTranX = 0;
    //当前是第几个视图
    this.nowView = parseInt(props.nowView, 10) || 0;

    this.reduceDistance = parseInt(props.reduceDistance, 10) || 0;//用于计算每个视图元素的宽度 itemAllWidth=windosWidth-reduceDistance;

    //每个视图元素的宽度
    this.itemWidth = parseInt(props.itemWidth || this.screenWidth - this.reduceDistance, 10);

    //当前视图位置
    this.nowTranX = -(this.itemWidth) * this.nowView + this.reduceDistance / 2;
  ...
  moveViewTo(viewIndex) {
    ...
    // console.log(this.data.list[this.nowView],this.nowView);
    this.nowView = viewIndex;

    this.nowTranX = -(this.itemWidth) * viewIndex + this.reduceDistance / 2;

    console.log('移动视图', viewIndex, this.nowTranX, this.itemWidth);

    // console.log(this.nowTranX)
